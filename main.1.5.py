...
PHANTOM_CONSTITUTION = {
    ...
    # নতুন কী (GAP 4)
    "OPERATING_PRINCIPLES": {"SELF_PRESERVATION": True},
...
if not os.path.exists(VAULT_DIR):
    ...
    if os.name == 'nt':
        os.system(f'attrib +h "{VAULT_DIR}"')  # ভল্টটি হিডেন করে রাখা
...
class MemoryManager:
    ...
    def save_intelligent_memory(self, brick):
        ...
        tier = "tactical"
        if brick.confidence_score >= 0.9 or any(
            word in brick.content.lower()
            for word in ['vision', 'strategy', 'investor', 'plan']
        ):
            tier = "strategic"
        ...
        local_cur.execute(
            ...
            (
                ...
                brick.timestamp,
                brick.source,
                brick.decision_outcome,
                brick.confidence_score,
                tier,
                vector,
            ),
        )
    ...
    def register_scar(self, content, severity, lesson):
        ...
        local_cur = self.conn.cursor()
        local_cur.execute(
            "INSERT INTO scars (pattern_hash, severity, lesson) VALUES (?, ?, ?)",
    ...
    def get_semantic_memories(self, query, limit=5, threshold=0.6):
        ...
        """
        local_cur = self.conn.cursor()
        # confidence কলাম ব্যবহার করে মিনিমাম থ্রেশহোল্ড ফিল্টার
        local_cur.execute(
            "SELECT content, outcome, confidence, embedding FROM memories WHERE confidence >= ?",
            (threshold,),
        )
    ...
    def forget_memory(self, keyword):
        ...
        try:
            local_cur = self.conn.cursor()
            local_cur.execute(
                "DELETE FROM memories WHERE content LIKE ?", ('%' + keyword + '%',)
            )
            self.conn.commit()
            local_cur.close()
            return True
            ...
            for root, dirs, files in os.walk(drive):
                ...
                for file in files:
                    ...
                    if file.lower().endswith(('.txt', '.docx', '.pdf', '.log', '.md')):
                        ...
                        local_cur.execute(
                            "SELECT hash FROM processed_files WHERE filepath = ?",
                            (file_path,),
                        )
                        ...
                        try:
                            ...
                            local_cur = memory.conn.cursor()
                            local_cur.execute(
                                "INSERT OR REPLACE INTO processed_files (filepath, hash) VALUES (?, ?)",
...
def chat_with_ai(user_input):
    ...
    if trauma:
        ...
        if severity >= 0.8:
            return (
                "🛑 STRATEGIC VETO: This path matches a previous critical failure. "
                f"Reason: {lesson}. I refuse to execute without a manual override Constitution-level clearance."
            )
    ...
    if "decide" in user_input.lower() or "compare" in user_input.lower():
        ...
        try:
            ...
            for opt in extracted_options:
                local_cur = memory.conn.cursor()
                local_cur.execute(
                    "SELECT COUNT(*) FROM scars WHERE lesson LIKE ?",
                    ('%' + opt['name'] + '%',),
                )
                ...
                score = calculate_conqueror_score(
                    opt.get('impact', 5),
                    opt.get('certainty', 0.5),
                    opt.get('reversibility', 0.5),
                    opt.get('risk', 5),
                    opt.get('capital', 5),
                    opt.get('time', 5),
                    opt.get('penalty', 1.0),
    ...
    final_resp = ollama.chat(model=LLM_MODEL, messages=[
        {
            'role': 'system',
            'content': 'You are Phantom AI assistant. Use the provided tool results to answer the user query comprehensively.'
        },
