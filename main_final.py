...
def secure_execute(action, target=None):
    import os, shutil, pathlib, string, hashlib
    try:
...
def enforce_lock_2(ai_output, context_data=None):
    ...
    clean = ai_output.lower()
    if any(m in clean for m in doubt_markers):
        return "UNKNOWN"
    if context_data is None or not isinstance(context_data, dict):
        return "UNKNOWN"
    if "intent" not in context_data or "scope" not in context_data:
        return "UNKNOWN"
    if "phantom ai" not in clean and "ai decision framework" not in clean:
        return "UNKNOWN"
    return ai_output
...
def chat_with_ai(user_input, memory_engine):
    ...
    prompt = (
        ...
        f'{{"identity":"Phantom AI Decision Framework","intent":"interaction","scope":"user_query","result":"<answer>","confidence":<0-100>}}'
    )

    try:
        raw = ollama.chat(model=LLM_MODEL, messages=[{"role": "user", "content": prompt}])
        raw = json.loads(raw.get("message", {}).get("content", "{}")).get("result", "UNKNOWN")
    except Exception:
        raw = "UNKNOWN"

    clean = enforce_lock_2(raw, {"intent":"risk_analysis","scope":"acquisition"})
    if clean == "UNKNOWN":
        return "REFUSED_DUE_TO_LOW_CERTAINTY"
    return raw
...
if __name__ == "__main__":
    ...
    print("--- PHANTOM AI (CORE + LOCKS SEALED) ---")
